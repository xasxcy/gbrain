...
describe('knobsHash determinism + cross-mode separation (CDX-4)', () => {
  ...
  test('KNOBS_HASH_VERSION constant exposed for migrations to bump on schema change', () => {
    ...
    // (rrc=) changes the text the cross-encoder scores, so a different
    // truncation threshold must not reuse cached rankings. Re-sequenced from
    // the fork's own v=29 (2026-09-07 merge) because upstream independently
    // claimed 29 for the ranker-wave knobs above — same D8 convention.
  ...
  test('#3515: detail set vs unset produces DIFFERENT hashes (cache contamination prevention)', () => {
    ...
    // Cat 13) — same unshipped epoch; a partial literal hashes as always.
    // 29→30 (fork, 2026-09-14 upstream sync): reranker_max_document_chars
    // (rrc=) re-sequenced past the upstream v=29 collision — same D8
    // convention.
    expect(KNOBS_HASH_VERSION).toBe(30);
  });
...
describe('v0.42.3.0 — autocut knobs', () => {
  test('KNOBS_HASH_VERSION is 30 (…; 25→26 salience/recency + intent_patterns fold #4415; 26→27 adaptive-return gate + intent fold E5b/F11; 27→28 compiledTruthBoost synthetic-row suppression #4256; 28→29 evb=/rrp=/kacf=/mbg= ranker-wave fold; 29→30 reranker document truncation rrc= (fork, re-sequenced past upstream v=29 collision))', () => {
    // 28→29: evb= expansion variant budget fold (ranker wave) — budget-weighted
    // variant fusion reorders rows for identical knobs; null hashes as legacy.
    // v=29 ALSO carries rrp= (relational rerank pin, ranker wave R1) — same
    // epoch, no extra bump: neither part had shipped in a release yet.
...
describe('ranker wave (R1) — relational_rerank_pin knob (relational rows bypass reranker demotion)', () => {
  ...
  test('knobsHash folds the pin (rrp=): 3 vs 0 vs 1 all differ; explicit 3 equals the bundle default', () => {
    ...
    // The pin rode KNOBS_HASH_VERSION 29 together with evb= (no separate
    // bump); 29→30 (fork, 2026-09-14 upstream sync) is an unrelated later
    // bump (rrc=) — this fact about rrp=/evb= sharing v=29 is still true,
    // just no longer the current version.
    expect(KNOBS_HASH_VERSION).toBe(30);
  });
});
