...
import * as invalidationModule from '../src/core/embedding-invalidation.ts';

// 2026-08-25 upstream merge: #4306 moved the signature-invalidation entry
// point off engine.invalidateStaleSignatureEmbeddings onto the standalone
// invalidateStaleSignatureEmbeddingsGuarded(). B5 below used to stub the
// engine method; after the merge that is a DEAD target — invalidation
// silently succeeded and B5's premise evaporated. Re-pointed at the module
// seam, gated by a flag so only B5 sees the failure (the #4283 probe-gated
// tests below need the real implementation). Mocking here rather than at
// engine.executeRaw is deliberate: rejecting a statement mid-transaction
// strands its connection and every later test in this file times out.
const realGuardedInvalidate = invalidationModule.invalidateStaleSignatureEmbeddingsGuarded;
let invalidationShouldThrow = false;
mock.module('../src/core/embedding-invalidation.ts', () => ({
  ...invalidationModule,
  invalidateStaleSignatureEmbeddingsGuarded: (...args: Parameters<typeof realGuardedInvalidate>) =>
    invalidationShouldThrow
      ? Promise.reject(new Error('invalidation unavailable'))
      : realGuardedInvalidate(...args),
}));
