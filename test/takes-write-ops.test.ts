/** Real authenticated MCP take mutations against coherent PGLite/file snapshots.
 * Pins holder authority, immutable resolutions, source routing, atomic publication,
 * and concurrent same-ID replay through the durable journal.
 */
...
import { dispatchToolCall as dispatch, type DispatchOpts } from '../src/mcp/dispatch.ts';
...
async function dispatchToolCall(db: PGLiteEngine, operation: string, params: Record<string, unknown>, opts: DispatchOpts) {
  const request = operation !== 'takes_scorecard' ? { request_id: randomUUID(), ...params } : params;
  return withVerifiedLocalRegistration(db, opts.remote === false ? cli : stdio,
    () => dispatch(db, operation, request, { config: { engine: 'pglite', embedding_disabled: true }, ...opts }));
}
async function mirrorFixture(slug: string, path = join(repo, `${slug}.md`), sourceId = 'default') {
  const snapshot = (await engine.readPageSnapshot(slug, { sourceId }))!;
  mkdirSync(dirname(path), { recursive: true });
  writeFileSync(path, serializePageToMarkdown(snapshot.page, snapshot.tags), 'utf8');
}
async function seedFixture(slug: string, body = `about ${slug}`, path = join(repo, `${slug}.md`), sourceId = 'default') {
  await engine.putPage(slug, { type: 'note', title: slug, compiled_truth: body }, { sourceId });
  await mirrorFixture(slug, path, sourceId);
}
...
beforeAll(async () => {
  ...
  for (const slug of ['people/alice-example', 'companies/acme-example', 'notes/lockrace', 'notes/registered-root']) {
    await seedFixture(slug);
  }
});
...
afterAll(async () => {
  ...
  for (const root of [repo, ...extraRoots]) rmSync(root, { recursive: true, force: true });
});
...
describe('takes_add', () => {
  test('creates the fence on first take [CEO-F6], mirrors to DB, returns the fence row number', async () => {
    ...
    // A committed receipt includes the canonical file and row together.
  ...
  test('registered canonical root remains authoritative if legacy sync.repo_path is unset', async () => {
    ...
    try {
      const result = parsed(await dispatchToolCall(engine, 'takes_add', {
        slug: 'notes/registered-root', claim: 'Owned root survives configuration drift', kind: 'fact', holder: 'world',
      }, { ...STDIO_WORLD }));
      expect(result.state).toBe('committed');
      expect(pageMd('notes/registered-root')).toContain('Owned root survives configuration drift');
    } finally { await engine.setConfig('sync.repo_path', repo); }
  });
  ...
  test('unknown page refuses with a terminal page identity receipt', async () => {
    const res = await dispatchToolCall(engine, 'takes_add', {
      slug: 'missing/page', claim: 'x', kind: 'fact', holder: 'world',
    }, { ...STDIO_WORLD });
    expect(parsed(res).error).toBe('page_identity_changed');
    expect(parsed(res).write_request.state).toBe('conflict');
  });
});

describe('takes_update + holder row fence', () => {
  test('updates mutable fields and commits the canonical file and row together', async () => {
...
describe('takes_resolve', () => {
  ...
  test('resolves a Git-root source_path when local_path is a repo subdirectory', async () => {
    ...
    await seedFixture(slug, [TAKES_FENCE_BEGIN,
      '| # | claim | kind | who | weight | since | source |',
      '|---|-------|------|-----|--------|-------|--------|',
      '| 1 | The scoped path resolves | take | world | 0.8 | 2026-08 | test |',
      TAKES_FENCE_END].join('\n'), filePath, sourceId);
...
describe('takes-write guards (invalid_input / row_inactive / md-absent / mcp_resolved scoping)', () => {
  ...
  test('md-file-absent: canonical file disappearance refuses without creating a replacement', async () => {
    const res = await dispatchToolCall(engine, 'takes_add', {
      slug: 'notes/md-absent', claim: 'Must not replace missing file', kind: 'fact', holder: 'world',
    }, { ...STDIO_WORLD });
    expect(res.isError).toBe(true);
    const body = parsed(res);
    expect(body.error).toBe('source_changed');
    expect(body.write_request.state).toBe('conflict');
  });
...
describe('takes-write adversarial regressions (F1 cross-holder / P1-2 containment / P1-4 atomic rollback / F2 strikethrough / P1-1 per-source)', () => {
  ...
  beforeAll(async () => {
    ...
    await engine.putPage(F1_SLUG, { type: 'note', title: F1_SLUG,
      compiled_truth: readFileSync(join(repo, `${F1_SLUG}.md`), 'utf8').replace(`# ${F1_SLUG}\n`, '').trim() });
    await mirrorFixture(F1_SLUG);
    // F2 + P1-4 start as fence-less pages (add appends the fence).
    for (const slug of [F2_SLUG, P14_SLUG]) {
      await mirrorFixture(slug);
    }
  });
  ...
  test('P1-2: a symlinked page dir that escapes the repo root is refused; nothing is written outside the tree', async () => {
    ...
    expect(res.isError).toBe(true);
    const body = parsed(res);
    // Registered-root containment is checked before any publication.
    expect(body.error).toBe('source_changed');
    expect(body.write_request.state).toBe('conflict');
  ...
  test('P1-4: a structured take failure rolls back the canonical file, revision, and rows; replay stays failed', async () => {
    const original = engine.addTakesBatch;
    const before = (await engine.readPageSnapshot(P14_SLUG))!;
    const bytes = pageMd(P14_SLUG);
    const params = { request_id: randomUUID(), slug: P14_SLUG,
      claim: 'Must not survive an incomplete publication', kind: 'fact', holder: 'world' };
    engine.addTakesBatch = async function () { throw new Error('simulated DB mirror failure'); };
    let result;
    try { result = parsed(await dispatchToolCall(engine, 'takes_add', params, { ...STDIO_WORLD })); }
    finally { engine.addTakesBatch = original; }
    expect(result.error).toBe('storage_error');
    expect(result.write_request.state).toBe('failed');
    expect(result.write_request.request_id).toBe(params.request_id);
    expect(pageMd(P14_SLUG)).toBe(bytes);
    expect((await engine.readPageSnapshot(P14_SLUG))!.revision).toBe(before.revision);
    expect(await engine.listTakes({ page_slug: P14_SLUG })).toHaveLength(0);
    const replay = parsed(await dispatchToolCall(engine, 'takes_add', params, { ...STDIO_WORLD }));
    expect(replay.error).toBe('storage_error');
    expect(replay.write_request.request_id).toBe(params.request_id);
    expect(pageMd(P14_SLUG)).toBe(bytes);
    expect((await engine.readPageSnapshot(P14_SLUG))!.revision).toBe(before.revision);
  });
  ...
  test('P1-1: a source with its own local_path publishes only under its registered root', async () => {
    ...
    const sourceId = 'takes-own-root';
    await engine.executeRaw('INSERT INTO sources(id,name,local_path) VALUES($1,$1,$2)', [sourceId, sourceRoot]);
    const file = join(sourceRoot, `${P11_SLUG}.md`);
    await seedFixture(P11_SLUG, 'Own source page', file, sourceId);
    const res = await dispatchToolCall(engine, 'takes_add', {
    ...
    expect(parseTakesFence(readFileSync(file, 'utf8')).takes.map(take => take.claim))
      .toEqual(['lands under the source local_path']);
    expect(await engine.listTakes({ page_slug: P11_SLUG, sourceId: 'default' })).toHaveLength(0);
    expect(await engine.listTakes({ page_slug: P11_SLUG, sourceId })).toHaveLength(1);
  });
});

describe('concurrency [ENG-E4/EV11]', () => {
  test('concurrent adds and same-ID replay retain both original claims exactly once', async () => {
    const requests = ['first', 'second'].map(claim => ({ request_id: randomUUID(),
      slug: 'notes/lockrace', claim: `${claim} immutable claim`, kind: 'fact', holder: 'world' }));
    const results = await Promise.all(requests.map(params => dispatchToolCall(engine, 'takes_add', params, { ...STDIO_WORLD })));
    for (let index = 0; index < requests.length; index++) {
      let body = parsed(results[index]);
      for (let attempt = 0; body.error === 'write_pending' && attempt < 3; attempt++) {
        body = parsed(await dispatchToolCall(engine, 'takes_add', requests[index], { ...STDIO_WORLD }));
      }
      expect(body.state).toBe('committed');
      expect(body.request_id).toBe(requests[index].request_id);
      const replay = parsed(await dispatchToolCall(engine, 'takes_add', requests[index], { ...STDIO_WORLD }));
      expect(replay.request_id).toBe(body.request_id);
      expect(replay.row_num).toBe(body.row_num);
      expect(replay.revision).toBe(body.revision);
    }
    const fence = parseTakesFence(pageMd('notes/lockrace'));
    expect(fence.takes.map(take => take.rowNum).sort()).toEqual([1, 2]);
    expect(fence.takes.map(take => take.claim).sort()).toEqual(requests.map(request => request.claim).sort());
    expect(await engine.listTakes({ page_slug: 'notes/lockrace' })).toHaveLength(2);
  });
});
...
describe('takes-write recorded source_path routing (#3605)', () => {
  test('add/update/resolve hit the recorded source_path file; no slug-derived stray', async () => {
    ...
    await mirrorFixture(slug, filePath);
    const add = parsed(await dispatchToolCall(engine, 'takes_add', {
    ...
    // Update and resolve publish to the same recorded canonical file.
  ...
  test('a captured page (file:// source_uri, no source_path) appends to the captured file', async () => {
    ...
    await mirrorFixture(slug, capturedAbs);
    const add = parsed(await dispatchToolCall(engine, 'takes_add', {
