...
describe('#3037 — one bad chunk no longer darkens its page', () => {
  test('--stale: siblings of one bad chunk get embedded; failure counted; signature not stamped', async () => {
    ...
    // fork: --stale routes through the atomic persistEmbedOutcome checkpoint
    // (test/embed.serial.test.ts:1095 pins this), not a direct upsertChunks
    // call — mock the checkpoint instead of upsertChunks.
    ...
    // Pre-fix: the batch threw, the checkpoint never ran, embedded stayed 0.
    expect(persistCalls).toHaveLength(1);
    const byIdx = new Map<number, any>(persistCalls[0].entries.map((e: any) => [e.chunkIndex, e]));
    expect(byIdx.get(0)!.outcome.vector).toBeInstanceOf(Float32Array);
    expect(byIdx.get(2)!.outcome.vector).toBeInstanceOf(Float32Array);
    expect(byIdx.get(1)!.outcome.failure).toBeDefined(); // bad chunk stays NULL (re-run picks it up)
  ...
  test('--stale x #3507: fan-out retries the WRAPPED texts and a partially-failed page is not restamped', async () => {
    ...
    const engine = mockEngine({
      ...
      getChunks: async () => THREE_CHUNKS,
      upsertChunks: async () => {},
      // fork: --stale routes through the atomic persistEmbedOutcome
      // checkpoint (test/embed.serial.test.ts:1095), not upsertChunks.
      persistEmbedOutcome: async (request: any) => {
        const vectors = request.entries.filter((entry: any) => 'vector' in entry.outcome).length;
        return { committedChunks: vectors, vectorCommittedChunks: vectors, staleSkippedChunks: 0, ledgerUpserts: 0, ledgerDeletes: 0 };
      },
