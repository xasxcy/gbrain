...
// FORK: exported so src/core/fork-migrations.ts can type its own bodies.
...
// byte-identical and the sync script never has to carry migration text.
// MUST run before LATEST_VERSION is computed — that is a Math.max over this
// same array, so a push that lands after it silently drops every fork
// migration out of `pending`. test/fork-migrations-registry.test.ts pins it.
MIGRATIONS.push(...buildForkMigrations({
