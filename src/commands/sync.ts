...
async function performSyncInner(engine: BrainEngine, opts: SyncOpts): Promise<SyncResult> {
  ...
  // Source config exclusions retain their established directory/prefix semantics;
  // CLI --exclude is handled below against the scope-relative path.
  ...
  if (importsToDo.length > 0) {
    ...
    // Git-diff paths are git-root-relative, including scoped sources.
