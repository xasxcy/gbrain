...
default): when the brain's ACTIVE reranker — an explicit
`search.reranker.model zeroentropyai:*` row (the pre-v0.48.2 mode-bundle
default; since v0.48.2 the bundle default is `voyage:rerank-2.5`, so a brain
with no reranker row is already off ZE) — is exposed, and the target provider ships a reranker, the
...
search still works, ordering quality drops. Past the shutdown date gbrain
skips the rerank call entirely (one-time stderr notice + a
`sunset_short_circuit` audit row), so searches pay no timeout.
