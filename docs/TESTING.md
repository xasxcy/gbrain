...
- `test/embed-oversize-heal-drain.serial.test.ts` — the heal wired into the `embedStalePages` phase-end-closure drain against PGLite: oversized stored rows split in place while the embedded sibling's vector survives. FORK DIVERGENCE (FORK_BACKLOG FB-004): the fork routes `embedStaleForSource` / `embedAllStale` (`embed --stale --all`, minion cursor-backfill) through the `persistStaleSlice` cursor-batched loop and does NOT port SUP-3874's in-place split there, so those two drains are not covered here (upstream's tests for them were dropped in the v0.46.29.0→v0.48.2.0 sync). `embedPage` / `embedStaleForSlugs` heal call-sites are fork-untouched and covered by their own tests.
